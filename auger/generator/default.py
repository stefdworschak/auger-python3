...
class DefaultGenerator(Generator):
            ...
            for _, calls in function.calls.items():
                for (args, _) in calls[:1]:
                    ...
                    for base in func_self.__class__.__bases__:
                        # For Python3 compatibility as __bases__ returns <class 'object'> whereas
                        # Python2 does not return anything
    ...
    def get_defining_item(self, code):
        ...
        for _,clazz in inspect.getmembers(mod, predicate=inspect.isclass):
            for _,member in inspect.getmembers(clazz, predicate=inspect.isfunction) + inspect.getmembers(clazz, predicate=inspect.ismethod):
    ...
    def dump_call(self, filename, code, call):
        ...
        for (args, return_value) in call:
            ...
            if isinstance(member, property) or (inspect.ismethod(member) or not is_mod):
                if not is_static:
